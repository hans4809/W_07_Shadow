...
void FGraphicsDevice::CreateSpotLightShadowMap()
{
    ...
    uint32 atlasH = Shadow::SPOT_LIGHT_RES * Shadow::SPOT_ATLAS_ROWS;
...
void FGraphicsDevice::CreatePointLightShadowMap()
{
    uint32 atlasW = Shadow::POINT_LIGHT_RES * Shadow::POINT_ATLAS_COLS;
    uint32 atlasH = Shadow::POINT_LIGHT_RES * Shadow::POINT_ATLAS_ROWS;
    ...
    texDesc.SampleDesc.Count = 1;                    
    texDesc.BindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE;
    Device->CreateTexture2D(&texDesc, nullptr, &PointShadowTextureAtlas);

    D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc = {};
    dsvDesc.Format = DXGI_FORMAT_D32_FLOAT;
    dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
    dsvDesc.Texture2D.MipSlice = 0;
    HRESULT hr1 = Device->CreateDepthStencilView(PointShadowTextureAtlas, &dsvDesc, &PointShadowDSV);
    ...
    srvDesc.Texture2D.MipLevels= 1;
    srvDesc.Texture2D.MostDetailedMip = 0;
    HRESULT hr2 = Device->CreateShaderResourceView(PointShadowTextureAtlas, &srvDesc, &PointShadowSRV);
}
